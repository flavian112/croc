...
static const int16_t tw64[64] = {
    ...
    30274, 12540,  28898, 15447,  27246, 18205,  25330, 20788,
    23170, 23170,  20788, 25330,  18205, 27246,  15447, 28898,
    12540, 30274,   9512, 31357,   6393, 32138,   3212, 32610,
        0, 32767,  -3212, 32610,  -6393, 32138,  -9512, 31357,
   -12540, 30274, -15447, 28898, -18205, 27246, -20788, 25330,
   -23170, 23170, -25330, 20788, -27246, 18205, -28898, 15447,
...
static void fft_sw_inplace(uint32_t *buf) {
    ...
    for (int i = 1, j = 0; i < FFT_N; i++) {
        ...
        j ^= bit;
        if (i < j) { uint32_t t = buf[i]; buf[i] = buf[j]; buf[j] = t; }
    }
    ...
    for (int half = 1; half < FFT_N; half <<= 1) {
        ...
        for (int k = 0; k < FFT_N; k += span) {
            for (int j = 0; j < half; j++) {
                ...
                buf[k + j]        = ((uint32_t)(uint16_t)(int16_t)((ar + wr) >> 1) << 16)
                                  | ((uint16_t)(int16_t)((ai + wi) >> 1));
                buf[k + j + half] = ((uint32_t)(uint16_t)(int16_t)((ar - wr) >> 1) << 16)
                                  | ((uint16_t)(int16_t)((ai - wi) >> 1));
            }
        }
    }
